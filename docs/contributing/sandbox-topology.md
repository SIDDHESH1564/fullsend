...
| `fullsend-code` | code, fix, review | Lower (code/fix are the least-run agents; review runs per-PR) | Go toolchain, scan-secrets, gopls, lychee, golangci-lint, pre-commit hook cache (`PRE_COMMIT_HOME`) |
